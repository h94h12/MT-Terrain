...
Vector3f Edge::bisection(float (*f)(Vector3f), Vector3f left, Vector3f right) {
	Vector3f mid = (right + left)/2;
	float leftV = f(left); float rightV = f(right); float midV = f(mid);
	if (abs(midV) < epsilon) {
		return mid;
	} else if (abs(leftV) > abs(rightV)) {
...
Tetrahedron::Tetrahedron(Vector3f *v1, Vector3f *v2, Vector3f *v3, Vector3f *v4, Grid *grid) {
	...
	CPC = 0;
	bool v1zero = abs(grid->func(*v1)) < 1e-30f;
	bool v2zero = abs(grid->func(*v2)) < 1e-30f;
	bool v3zero = abs(grid->func(*v3)) < 1e-30f;
	bool v4zero = abs(grid->func(*v4)) < 1e-30f;
	//bool z1, z2, z3, z4, z5, z6;
	/* if (v1zero && v2zero && v3zero && v4zero) {
		// KEEP CALM AND CARRY ON
	} else if (v1zero && v2zero && v3zero) {
		cutpoints[CPC] = *v1;
		cutpoints[CPC] = *v2;
		cutpoints[CPC] = *v3;
		CPC += 3;
	} else if (v1zero && v2zero && v4zero) {
		cutpoints[CPC] = *v1;
		cutpoints[CPC] = *v2;
		cutpoints[CPC] = *v4;
		CPC += 3;
	} else if (v1zero && v3zero && v4zero) {
		cutpoints[CPC] = *v1;
		cutpoints[CPC] = *v3;
		cutpoints[CPC] = *v4;
		CPC += 3;
	} else if (v2zero && v3zero && v4zero) {
		cutpoints[CPC] = *v2;
		cutpoints[CPC] = *v3;
		cutpoints[CPC] = *v4;
		CPC += 3;
...
void Tetrahedron::draw() {
	...
	if (drawTets) {
		...
		if (wireframe) {
			...
			glVertex3f(cutpoints[3](0), cutpoints[3](1), cutpoints[3](2));	
			glVertex3f(cutpoints[1](0), cutpoints[1](1), cutpoints[1](2));
			glVertex3f(cutpoints[0](0), cutpoints[2](1), cutpoints[2](2));
			glEnd();
		}		
	}
}
