...
Tetrahedron::Tetrahedron(Vector3f *v1, Vector3f *v2, Vector3f *v3, Vector3f *v4, Grid *grid) {
	...
	CPC = 0;
	if (abs(grid->func(*v1)) - epsilon < 0) {
		cutpoints[CPC] = *v1;
		CPC++;

	}
	if (abs(grid->func(*v2)) - epsilon < 0) {
		cutpoints[CPC] = *v2;
		CPC++;
	}
	if (abs(grid->func(*v3)) - epsilon < 0) {
		cutpoints[CPC] = *v3;
		CPC++;
	}
	if (abs(grid->func(*v4)) - epsilon < 0) {
		cutpoints[CPC] = *v4;
		CPC++;
	}
...
void Tetrahedron::draw() {
	if (drawTets) {
		...
		if (wireframe) {
			...
			glBegin(GL_LINE_LOOP);
			glVertex3f(cutpoints[4](0), cutpoints[4](1), cutpoints[4](2));
			glVertex3f(cutpoints[1](0), cutpoints[1](1), cutpoints[1](2));
			glVertex3f(cutpoints[2](0), cutpoints[2](1), cutpoints[2](2));			
			glEnd();
